...
public class HorTaperChart extends View {
    ...
    public HorTaperChart(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        mTextSize = typedArray.getDimensionPixelOffset(R.styleable.TaperChart_tc_axis_txtsize, mTextSize);
...
//                    }
//                }
                if(EmptyUtils.isEmpty(mChartBeans)){
                    ...
                    for (int i = 0; i < x_count; i++) {
                        ...
                        float dataPAxisH = mCanvasH - (value * oneH);
                        pf = new PointF();
                        pf.x = start_x + mLabelWidth * i - mInterSpace * i + mHorOffset; // 水平移动距离变化值
                        pf.y = pf_y;

                        // 对象组装
                        TaperChartBean bean = new TaperChartBean();
                        bean.setxValue(key);
                        bean.setyValue(value);
                        bean.setDataPAxisH(dataPAxisH);
                        bean.setPointF(pf);
                        bean.setPaintColor(COLORS[0]);
                        ...
                        // 绘制图形
                        drawGraph(canvas, pf, dataPAxisH, COLORS[0], key, value);
                    }
                }else{
                    int size = mChartBeans.size();
                    TaperChartBean bean;
                    for (int i = 0; i < size; i++) {


                        bean = mChartBeans.get(i);
                        if (bean.getPointF().x + mHorOffset < mFirstOffSet) {
                            bean.getPointF().x = -bean.getPointF().x - mLabelWidth;
                        } else {
                            bean.getPointF().x = bean.getPointF().x + mHorOffset;
                        }




                        drawGraph(canvas, bean.getPointF(), bean.getDataPAxisH(), COLORS[0], bean.getxValue(), bean.getyValue());
                    }
                }
                break;
        }
    ...
    private float mFirstOffSet; // 第一个峰值图的左侧偏移量
    private float mLastOffSet; // 最后一个峰值图的右侧偏移量
    private List<TaperChartBean> mChartBeans;
...
//            canvas.drawLine(0,mCanvasH - mOffBtm,mCanvasW,mCanvasH - mOffBtm,mPaint);
    ...
    private float currMoveX,lastMoveX;
    private long currentMS = 0l;
    private final float touchM = 24; // 判定移动距离的阈值
    private float mHorOffset; // 水平移动的偏移量
    @Override
...
//                float delet = scrollX - downX;
//                mHorOffset = delet;
//                downX = scrollX;
//                Common.log_d("201903251658", "方向 -> " + (delet > 0 ? "右滑" : "左滑") + "->水平值 = " + mHorOffset);
//                invalidate();


                currMoveX = event.getX();
                // 本次的滑动距离
                int scrolledX = (int) (lastMoveX - currentMS);
                // 如果右滑时 内容左边界超过初始化时候的左边界 就还是初始化时候的状态
                if (getScrollX() + scrolledX < mFirstOffSet) {
                    scrollTo((int) (-mFirstOffSet), 0);
                    return true;
                }
                // 同理 如果左滑  这里判断右边界
                else if (getScrollX() + getWidth() + scrolledX > mLastOffSet) {
                    scrollTo((int) (mLastOffSet - getWidth()), 0);
                    return true;
                } else {
                    //在左右边界中 自由滑动
